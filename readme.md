...
1. 在 `./admin/th.php` 中建立商品分類的表單，這邊我們採用form表單來傳遞資料再導回 `th.php` 即可，要用ajax也可以。
2. 撰寫新增大分類和新增中分類的程式碼，這邊我們利用到表單name屬性及表單傳送值的特性，直接儲存整個$_POST的陣列即可，這可以省去一個api程式的撰寫。
3. 在 `./admin/th.php` 中接著撰寫商品分類的列表，這邊示意圖是以主副分類的順序來顯示，因此我們採用巢狀迴圈的做法來列出所有的分類。
4. 接著從 `admin.php` 或是 `mem.php` 中複製 `del(table,id)` 函式過來，或是把這個函式搬到 `./js/js.js` 中成為全域函式也可以。
5. 最後撰寫編輯分類的功能，這邊我們使用js內建的prompt()來提供編輯的彈出畫面，然後再以ajax的方便將修改後的文字送到 `./api/edit_type.php` 去做資料表的更新
...
2. 在 `./admin/` 目錄中新增 `add_goods.php`，並撰寫新增商品需要的表單內容
3. 製作大分類和中分類的選單連動功能，這邊我們利用和題組三訂票功能一樣的ajax方式來達成
4. 在 `./api/` 目錄新增 `get_main.php` 及 `get_sub.php` 兩個檔案，分別撰寫取得大分類和中分類的選項內容回傳給前端選單使用
5. 在 `./api/` 目錄中新增 `add_goods.php`，並撰寫新增商品資料到資料表的程式碼，此時要記得產生一組商品編號後才寫入到資料表
6. 複製 `./admin/add_goods.php`，改名為 `edit_goods.php`，並將商品資料填入對應的表單欄位中，要記得加入隱藏欄位id
7. 複製 `./api/add_goods.php`，改名為 `edit_goods.php`，並撰寫更新資料需要的程式碼內容
...
3. 在 `./front/home.php` 中撰寫導航文字及撈出商品內容的程式碼，注意商品簡介只需要顯示固定長度的一部份字串即可。
4. 在 `home.php` 中要注意的是在使用者按下 `我要購買` 按鈕時，我們除了要把使用者導向購物車頁面外，也同時要把商品的`id`及`數量`一起帶過去。
5. 在 `./front/` 目錄中增加 `detail.php` 檔案，並且撰寫商品詳細內容的頁面。
6. 在詳細內容頁面中，要注意的是我們採用 **`get`**的方式來做表單的傳送，中間放了二個隱藏欄位`do`及`id`，這在表單送出後會在網址列形成 `index.php?do=buycart&id=3&qt=1`，進而達到把購買數量及商品id帶到購物車頁面的效果
...
#### 購物車功能
1. 先前在 `./api/chkpw.php` 中已經增加了一個session變數，用來紀錄使用者登入的狀況
...
6. 刪除購物車商品的功能我們採用ajax的方式來製作即可，將要刪除的商品id傳到後台api去，然後由api來刪除session中的商品內容，完成後再重新導向 `buycart.php` 一次並清除網址帶的商品參數，此時因為購物車內的商品數量減少了，所以列表就會少一樣商品，達成題目要求的功能
